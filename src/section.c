...
static ENCRY_INFO* SaveEncryInfo(ENCRY_INFO* const buffer, const DWORD rva,
                                 const DWORD size, const QWORD key);
...
DWORD EncryCallBack(const PE_IMAGE_INFO* const image_info,
    ...
    if (min_size > 0) {
		...
		byte* key = GenerateRandomKey(sizeof(QWORD));
        EncryptRC4(key, sizeof(QWORD), RvaToVa(image_info, rva), min_size);
        // Save the encryption information.
        ENCRY_INFO* buffer = *(ENCRY_INFO**)encry_info;
        *(ENCRY_INFO**)encry_info = SaveEncryInfo(buffer, rva, min_size, *((QWORD*)key));
