...
bool InstallShell(const PE_IMAGE_INFO* const image_info,
	...
	byte* load_seg_key = GenerateRandomKey(sizeof(QWORD));
    // Encrypt the load segment and transformed import table.
    EncryptRC4(load_seg_key, sizeof(QWORD), load_seg_base, load_seg_size + new_imp_table_size);
	...
	seg_encry_info->seg_key = *((QWORD*)load_seg_key);
